...
public class LogicGraphClassifierImpl implements LogicGraphClassifier {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(LogicGraphClassifierImpl.class);
	 ...
	 * 
	 * @param logicClassifierStoreIn
	 *            the store that is specific to ochre and lucene
	 */
	@Inject
	public LogicGraphClassifierImpl(
			final LogicClassifierStore logicClassifierStoreIn) {
	...
	public int getNidFromSNOMED(final String sctid) {
		TerminologyStoreDI termStore = logicClassifierStore
				.getTerminologyStore();
		TerminologySnapshotDI termSnapshot = termStore
				.getSnapshot(logicClassifierStore.getViewCoordinates());
	...
	public Integer classify(final LogicGraph logicGraph) {
		...
		Integer classifiedResult = logicService
				.getConceptSequenceForExpression(logicGraph,
	...
	@Override
	public LogicGraph getStatedTermLogicGraph(final int nid) throws IOException {
		TerminologySnapshotDI statedTermSnapshot = logicClassifierStore
				.getTerminologyStore().getSnapshot(
						ViewCoordinates.getDevelopmentStatedLatest());
		return logicClassifierStore.getLogicService().createLogicGraph(
				(ConceptVersion) statedTermSnapshot.getConceptVersion(nid));
	}
