...
public class FhirElementProcessorFactoryImplTest {
    ...
    public void factoryTest() throws ClassifierException {
        ...
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(adverseReaction), adverseReactionProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(alert), alertProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(allergyIntolerance), allergyIntoleranceProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(binary), binaryProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(carePlan), carePlanProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(composition), compositionProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(conceptMap), conceptMapProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(conceptMap), conceptMapProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(condition), conditionProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(conformance), conformanceProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(deviceObservationReport), deviceObservationReportProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(device), deviceProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(diagnosticOrder), diagnosticOrderProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(diagnosticReport), diagnosticReportProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(documentManifest), documentManifestProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(encounter), encounterProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(familyHistory), familyHistoryProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(group), groupProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(imagingStudy), imagingStudyProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(immunization), immunizationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(immunizationRecommendation), immunizationRecommendationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(list), listProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(location), locationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(media), mediaProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(medicationAdministration), medicationAdministrationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(medicationDispense), medicationDispenseProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(medicationDispense), medicationDispenseProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(medicationPrescription), medicationPrescriptionProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(medication), medicationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(medicationStatement), medicationStatementProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(messageHeader), messageHeaderProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(observation), observationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(operationOutcome), operationOutcomeProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(order), orderProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(orderResponse), orderResponseProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(orgazation), organizationProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(other), otherProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(patient), patientProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(practitioner), practitionerProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(procedure), procedureProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(profile), profileProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(provenance), provenanceProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(query), queryProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(questionnaire), questionnaireProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(relatedPerson), relatedPersonProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(securityEvent), securityEventProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(specimen), specimenProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(substance), substanceProcessor);
        Assert.assertEquals(legoElementProcessorFactory.findClassifier(valueSet), valueSetProcessor);
    }
    ...
    public void factoryFailureTest() throws ClassifierException {
        ...
        FhirElementProcessorFactory legoElementProcessorFactory =
                new FhirElementProcessorFactoryImpl(iterableProvider);
        legoElementProcessorFactory.findClassifier(new Object());
    }
}
