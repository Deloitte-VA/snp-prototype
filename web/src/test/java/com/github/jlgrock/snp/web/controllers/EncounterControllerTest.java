...
public class EncounterControllerTest extends GenericControllerTest {
    ...
    public void testGetEncounterMethod() {
        Mockito.when(encounter.getId()).thenReturn(ObjectId.get());
    ...
    public void testGetEncounterRestCall() throws IOException {
        ...
        ObjectId encounterId = ObjectId.get();
        encounterTemp.setId(encounterId);
        ObjectId patientId = ObjectId.get();
        encounterTemp.setPatientId(patientId);
        ...
        final WebTarget target = target("encounter/" + encounterId.toString());
